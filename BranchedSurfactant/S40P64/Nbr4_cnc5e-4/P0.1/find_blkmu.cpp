...
//  printf("%d \t %.15f\n",i,fblkr[i]);
 ...
 tmprhoN3 = fblkr[TNsp-2]+fblkr[TNsp-1];
  ...
  for(i=0; i< TNsp ; i++)
  {
   ...
   if(fblkr[i] >0 )
   {
    ...
    else
    {
     ...
     {
      blkdfid[i] = (1.0-Xa)*log(tmprhoN3/tmpNN3)/tmpNN3;
      blkfid[i] = (1.0-Xa)*tmprhoN3*(log(tmprhoN3/tmpNN3)-1)/tmpNN3;
      tmpfid = tmpfid+blkfid[i];
      //printf("%d \t %.15f \t %.15f\n",i,blkdfid[i],blkfid[i]);
     }
    }
...
//     printf("%d \t %.15f\n",i,blkdfhs[i]);
    ...
    {
     blkdfhs[i] = (1.0 - Xa)*PI*blkdfhsn3[i]/6*pow(sigma[i],3);
//     printf("%d \t %.15f\n",i,blkdfhs[i]);
    }
...
//   printf("3a-3b \t %.15f\n",blkdfhs[TNsp-2]+blkdfhs[TNsp-1]);


   //calculate values from fch
   double blkfch, blkdfchn3[TNsp],blkdfch[TNsp],tmp_rsigab,tmp_sigab,tmp_gab,tmp_lgab,tmp_alpha,tmp_beta,tmp_dd0,tmp_dd1,tmp_dd2;
   

   tmp_rsigab = rsigma[TNsp-1]*rsigma[TNsp-2]/(rsigma[TNsp-1]+rsigma[TNsp-2]);
   printf("%.15e \t %.15e \t %.15e \n",tmp_rsigab,rsigma[TNsp-1],rsigma[TNsp-2]); 
   tmp_sigab = tmp_rsigab*(1. - 0.12*exp(-3.0*crsepsln[crsTNsp-1]/Tmpr));
   printf("%.15e \t %.15e \t %.15e \n",tmp_sigab,sigma[TNsp-1],sigma[TNsp-2]);
...
//   printf("%.15e \t %.15e \t %.15e\n", tmp_gab,tmp_gg[TNsp-1],tmp_gg[TNsp-2]); 	

   tmp_alpha = blkn3[TNsp-2]/pow(sigma[TNsp-2],3) + blkn3[TNsp-1]/pow(sigma[TNsp-1],3);
   ...
   blkfch =0 ;
   blkfch = -(tmprhoN3/tmpNN3)*( (1. - double(Nbr))*log(tmp_gg[TNsp-1]) + double(Nbr)*log(tmp_gab));
   for(i =0; i<TNsp; i++)
   {
     tmp_lg = log(tmp_gg[i]);
     ...
     {
      blkdfch[i] =  (1.0 - Xa)*PI*pow(sigma[i],3)/6*blkdfchn3[i];
      printf("%d \t %.15f\n",i,blkdfch[i]); 
     }
     else
     {
...
//    printf("Fdisp1:%.15f\n",blkfdisp1);
    ...
    for(i=0;i<TNsp;i++)
    {
        ...
        {
         blkdfdisp1[i] =  (1.0-Xa)*(blkdfdisp1n0[i] + PI/6*pow(sigma[i],3)*blkdfdisp1n3);
        }
        else
        {
...
//    printf("Fidps:%.15f\n",blkfdisp2+blkfdisp1);
    ...
    for(i=0;i<TNsp;i++)
    {
        ...
        {
        	blkdfdisp2[i] = (1.0 - Xa)*(blkdfdisp2n0[i] + PI/6*pow(sigma[i],3)*blkdfdisp2n3);
        }
        else
        {
  ...
  //  printf("dfdisp-3a3b:%.15f\n",blkdfdisp1[TNsp-1] +blkdfdisp2[TNsp-1]+blkdfdisp1[TNsp-2] +blkdfdisp2[TNsp-2]); 
