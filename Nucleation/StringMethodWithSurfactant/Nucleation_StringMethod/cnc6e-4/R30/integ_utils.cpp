...
double integ_inpd( int np, double* dat1, double* dat2 ) {

  int i, j; 
  double tph2 = dx[Dim-1]*dx[Dim-1],sum=0;
  ...
  sum = 3.0*dat1[np-1]*dat2[np-1]/8.0 +  7.0*dat1[1]*dat2[1]/6.0 + 23.0*dat1[2]*dat2[2]/24.0;

#pragma omp parallel for reduction(+:sum)
  for (i=3; i<np-3; i++)
	sum += dat1[i]*dat2[i];

  sum += 23.0*dat1[np-3]*dat2[np-3]/24.0 + 7.0*dat1[np-2]*dat2[np-2]/6.0 + 3.0*dat1[np-1]*dat2[np-1]/8.0;
...
double integ_inpd( int np, double* dat1, double* dat2 ) {
  ...
  sum =  7.0*dat1[1]*dat2[1]/6.0 + 4*23.0*dat1[2]*dat2[2]/24.0;

#pragma omp parallel for reduction(+:sum)
  for (i=3; i<np-3; i++)
	sum += dat1[i]*dat2[i]*i*i;

  sum += 23.0*dat1[np-3]*dat2[np-3]/24.0*(np-3)*(np-3) + 7.0*dat1[np-2]*dat2[np-2]/6.0*(np-2)*(np-2) + 3.0*dat1[np-1]*dat2[np-1]/8.0*(np-1)*(np-1);





  return pow(dx[Dim-1],2)*4*PI*sum;
...
double integ_simpson_sph(int np, double* dat){
  ...
  sum = 7.0*dat[1]/6.0 + 4.*23.0*dat[2]/24.0;

#pragma omp parallel for reduction(+:sum)
  for (i=3; i<np-3; i++)
	sum += dat[i]*i*i;

  sum += pow(np-3,2)*23.0*dat[np-3]/24.0 + pow(np-2,2)*7.0*dat[np-2]/6.0 + pow(np-1,2)*3.0*dat[np-1]/8.0;
