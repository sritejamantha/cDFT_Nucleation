...
void initialize() {
 ...
 //int mid_ind =110;
  ...
  FILE *f0,*f1,*f2;
  f0 = fopen("rhoPl_Inp.dat","r");
  f1 = fopen("rhoS_Inp.dat","r");
  f2 = fopen("rhoP_Inp.dat","r");
//  printf("%lf \t %d \t %lf \t %d\n",RgA,NRgA,RgB,NRgB);
  ...
  for(j=0;j<M;j++)
  {
   ...
   fscanf(f1,"%lf %lf",&xInp,&rhoK[2][j]);
   fscanf(f2,"%lf %lf",&xInp,&rhoK[3][j]);
   rhoK[1][j] = rhoK[3][j]*cofac;
  }
