...
void read_input( void ) {
  ...
  fscanf(inp, "%lf %lf",&rhoLA,&rhoLB);
  printf("LqDensPDMS:%lf \t LqDensSolvPhil:%lf\n",rhoLA,rhoLB);
  fgets( tt , 80 , inp) ;
  fgets( tt , 80 , inp) ;
  ...
  fscanf( inp , "%d %d" , &otp_freq, &otp_freq2 ) ;
  fgets( tt , 80 , inp) ;
  printf("The ttstep  = %d and otp freq = %d, otp freq2 = %d\n",ttstep,otp_freq,otp_freq2);
