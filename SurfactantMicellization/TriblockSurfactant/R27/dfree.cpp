...
#pragma omp parallel for private(nn,k,l,ind1,ind2,indz)
  ...
  /*for(j=0;j<M;j++)
  {
   printf("%d \t %.15e \t %.15e \t %.15e\n",j,dfloc[0][j],dfloc[1][j],dfloc[2][j]);
  }*/
...
/*  for(j=0;j<M;j++)
  {
   printf("%d \t %.15e \t %.15e \t %.15e\n",j,dfdispndr[0][j],dfdispndr[1][j],dfdispndr[2][j]);
  }

  exit(-1); */ 
  tmpNN3 = NK[TNsp-3]+NK[TNsp-2]+NK[TNsp-1];
  for(i=0;i<M;i++)
  {

	ttF[i] =0;
        tmprhoN3=rhoK[TNsp-3][i]+rhoK[TNsp-2][i]+rhoK[TNsp-1][i];
	for(j=0;j<TNsp;j++)
        {
		if(j==TNsp-3)
                {
                ...
                {
                 ttF[i] += Xbr[i]*tmprhoN3/tmpNN3*(log(tmprhoN3/tmpNN3 + 1.e-30)-1.);
                 ttF[i] += dfdispndr[j][i]*rhoK[j][i];
                ...
                {
                 ttF[i] += (1.0 - Xar[i] - Xbr[i])*tmprhoN3/tmpNN3*(log(tmprhoN3/tmpNN3 + 1.e-30)-1.);
                 ttF[i] += dfdispndr[j][i]*rhoK[j][i];
...
//        printf("%d \t %.15e\n",i,dfttdrho[TNsp-2][i]+dfttdrho[TNsp-1][i]);
...
#pragma omp parallel for private(tmp_gg,tmp_lg,tmp_1m,tmp_eta,tmp_dd,i,k,nn,tmp_gab,tmprhoN3,tmpch_cf3b,tmpch_cf3ab,tmp_lgab,tmp_alpha,tmp_beta,tmp_dd0,tmp_dd1,tmp_dd2)
	...
	tmp_1m = 1.-wn3[TNsp][j];
        ...
        tmprhoN3 = wn0[TNsp-3][j]+wn0[TNsp-2][j]+wn0[TNsp-1][j];
	...
	for(i =0 ;i<TNsp; i++)
  	...
  	for(i =0 ;i<TNsp; i++)
        ...
        tmp_alpha = wn3[TNsp-3][j]/pow(sigma[TNsp-3],3) + wn3[TNsp-2][j]/pow(sigma[TNsp-2],3) + wn3[TNsp-1][j]/pow(sigma[TNsp-1],3);
   ...
   Fch[j] = -(tmprhoN3/tmpNN3)*(1. - double(Nbr))*( log(tmp_gg[TNsp-2]) + log(tmp_gg[TNsp-1]) );
   Fch[j] = Fch[j] - (tmprhoN3/tmpNN3)*double(Nbr)*( log(tmp_gab) + log(tmp_gbc) );
  ...
  for(j=0; j<M; j++)
  {
	...
	tmp_eta = wn3[TNsp][j];
        ...
        for(i=0;i<TNsp;i++)
        {
                ...
                {
                  loc_x[i] = Xb*wn0[i][j]/NK[i];
                }
                else if(i==TNsp-1)
                {
                  loc_x[i] = (1.0-Xa-Xb)*wn0[i][j]/NK[i];
                }
                else
                {
