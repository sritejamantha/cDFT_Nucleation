...
void poly_density()
{
    int adj_len,i,j,k,l,fl,ind1,ind2,indz,nn[Dim];
    int tmpTNsp,tmpNs,tmpsigNab,tmpsigNbc,Nbp,tmpsigNhfab,tmpsigNhfbc;    
    double *q_tmp,k2, vect[Dim],cent_v,tmpN2sigab,tmp_rsigab=0.0,tmp_sigab=0.0,mubp=0.0;
    double tmp_rsigbc=0.0,tmp_sigbc=0.0,tmpN2sigbc;
...
//    tmpNs = NK[TNsp-1]/Nbr;
    ...
    tmp_rsigbc = (rsigma[TNsp-2]+rsigma[TNsp-1])/2.0;
    tmp_sigbc = tmp_rsigab*(1. - 0.12*exp(-3.0*crsepsln[crsTNsp-1]/Tmpr));

    tmpsigNab = round(tmp_sigab/dx[0]);
    tmpN2sigab = 2.0*round(tmp_sigab/dx[0])*dx[0];

    tmpsigNbc = round(tmp_sigbc/dx[0]);
    tmpN2sigbc = 2.0*round(tmp_sigbc/dx[0])*dx[0];

    tmpsigNhfab = round(tmp_sigab/2./dx[0]);
    tmpsigNhfbc = round(tmp_sigbc/2./dx[0]);
...
#pragma omp parallel for private(k,ind1,ind2)
    for(j=0;j<M;j++)
    {
     ...
     if(nn[Dim-1] < sigNhf[TNsp-3])
        continue;

     int tid = omp_get_thread_num() ;
     if(j < sigN[TNsp-3])
     {
       ind1 =  padN2 + sigN[TNsp-3] - j;
     ...
     qs_fwd_pgt[l][j] = dx[Dim-1]*integ_simpson(adj_len,tmpN2[tid])/N2sig[TNsp-3]/extz[j+padN2];

    }//j  M

    cent_v = q_tmp[padN2 +sigN[TNsp-3]] ;

    interp_values(qs_fwd_pgt[l],cent_v,sigNhf[TNsp-3]);
...
#pragma omp parallel for private(k,ind1,ind2)
   for(j=0;j<M;j++)
   {
     ...
     if(nn[Dim-1] < tmpsigNhfab)
       continue;

    int tid = omp_get_thread_num() ;
    if(j < tmpsigNab)
    {
      ind1 =  padN2 + tmpsigNab - j;
      ind2 = j+padN2 + tmpsigNab;
      adj_len = 2*j +1;
    }
    else
    {
      ind1 = j+padN2 - tmpsigNab;
      ind2 = j+padN2 + tmpsigNab;
      adj_len = 2*tmpsigNab+1;
    }
    for(k=0;k<adj_len;k++)
    {
      tmpN2[tid][k] = q_tmp[ind1+k]*extz[ind1+k];
    }//k
    qs_fwd_pgt[l][j] = dx[Dim-1]*integ_simpson(adj_len,tmpN2[tid])/tmpN2sigab/extz[j+padN2];

   }//j  M

   cent_v = q_tmp[padN2 +tmpsigNab] ;

   interp_values(qs_fwd_pgt[l],cent_v,tmpsigNhfab);
...
#pragma omp parallel for private(k,ind1,ind2) 
   for(j=0;j<M;j++)
   {
     ...
     if(nn[Dim-1] < tmpsigNhfbc)
       continue;

    int tid = omp_get_thread_num() ;
    if(j < tmpsigNbc)
    {
      ind1 =  padN2 + tmpsigNbc - j;
    ...
    else
    {
      ...
      ind2 = j+padN2 + tmpsigNbc;
      adj_len = 2*tmpsigNbc+1;
    }
    for(k=0;k<adj_len;k++)
    {
      tmpN2[tid][k] = q_tmp[ind1+k]*extz[ind1+k];
    }//k
    qs_fwd_pgt[l][j] = dx[Dim-1]*integ_simpson(adj_len,tmpN2[tid])/tmpN2sigbc/extz[j+padN2];

   }//j  M

   cent_v = q_tmp[padN2 +tmpsigNbc] ;

   interp_values(qs_fwd_pgt[l],cent_v,tmpsigNhfbc);
...
#pragma omp parallel for private(k,ind1,ind2)
   for(j=0;j<M;j++)
   {
    ...
    qs_bkd_pgt[l][j] = dx[Dim-1]*integ_simpson(adj_len,tmpN2[tid])/tmpN2sigbc/extz[j+padN2];

   }//j  M

   cent_v = q_tmp[padN2 +tmpsigNbc] ;

   interp_values(qs_bkd_pgt[l],cent_v,tmpsigNhfbc);
...
#pragma omp parallel for private(k,ind1,ind2)
   for(j=0;j<M;j++)
   {
    ...
    qs_bkd_pgt[l][j] = dx[Dim-1]*integ_simpson(adj_len,tmpN2[tid])/tmpN2sigab/extz[j+padN2];

   }//j  M

   cent_v = q_tmp[padN2 +tmpsigNab] ;

   interp_values(qs_bkd_pgt[l],cent_v,tmpsigNhfab);
...
#pragma omp parallel for private(k,ind1,ind2)
    for(j=0;j<M;j++)
    {
     ...
     qs_bkd_pgt[l][j] = dx[Dim-1]*integ_simpson(adj_len,tmpN2[tid])/N2sig[TNsp-3]/extz[j+padN2];

    }//j  M

    cent_v = q_tmp[padN2 +sigN[TNsp-3]] ;

    interp_values(qs_bkd_pgt[l],cent_v,sigNhf[TNsp-3]);
